...
action(market,portfolio){if(this.level>0){this.behaviour(this,market,portfolio)}}}},{}],2:[function(require,module,exports){const Bot=require('./Bot.js');module.exports=class{constructor(market,portfolio){this.portfolio=portfolio.model;this.market=market.model;this.bots=[]}
...
update(){this.bots.forEach(current=>{current.action(this.market,this.portfolio)})}
upgrade(bot){if(this.portfolio.cash>=bot.costs[bot.level]){new Audio('./sounds/robot.mp3').play()
this.portfolio.cash-=bot.costs[bot.level];bot.level ++}}
...
$('body').append(DialogWindow);$('#resume').click(()=>{loop.pause();$('#help').remove()})}},{}],5:[function(require,module,exports){module.exports={name:"Auto-Buy",description:"This bot will automatically buy stocks when they are about to make a lot of money",costs:[100,200,500,3000],behaviour:function(bot,market,portfolio){const buyingCaps=[10,20,30,40,50];let currentCap=(portfolio.cash/100)*buyingCaps[bot.level];market.iterate((current)=>{if(current.price<currentCap&&current.momentum>0.6){currentCap-=current.price;portfolio.buy(current,1)}})}}},{}],6:[function(require,module,exports){module.exports={name:"Auto-Sell",description:"This bot will automatically sell stocks when they are crashing or when they are thriving",costs:[100,200,1000,3000],behaviour:function(bot,market,portfolio){portfolio.stocks.forEach(current=>{if(current.stock.momentum<0){portfolio.sell(current.stock,current.quantity)}})}}},{}],7:[function(require,module,exports){const View=require('./../View/BotShop.js');const Model=require('./../Model/BotShop.js');class BotShopController{constructor(Loop,Portfolio,Market,Calendar){this.Loop=Loop;this.Portfolio=Portfolio;this.Market=Market;this.Model=new Model(Market,Portfolio);this.View=new View(this.Model,Loop,Calendar.model);this.Loop.addViewItem(this.View);this.Loop.addRepeating(()=>{this.Model.update()},3000)}
...
return PortController}},{"./../model/Portfolio.js":18,"./../view/Portfolio.js":26}],13:[function(require,module,exports){$(document).ready(function(){let GameConsole=require('./view/Console.js')();let Loop=require('./model/Loop.js')();let GameSave=store.get('GameSave');let Market=require('./controller/Market.js')(Loop,GameSave);let Portfolio=require('./controller/Portfolio.js')(Loop,Market,GameConsole,GameSave);let Calendar=require('./controller/Calendar.js')(Loop,Portfolio,GameConsole,GameSave);let OddJobs=require('./controller/OddJobs.js')(Loop,Portfolio,Calendar);let Broker=require('./controller/Broker.js')(Loop,Market,Portfolio,Calendar);let BotShop=require('./controller/BotShop.js')(Loop,Portfolio,Market,Calendar,GameSave);Market.setCalendar(Calendar);Loop.addRepeating(()=>{GameConsole.message('Auto-Saver: Your game was saved')
let saveGame={};saveGame.Market=Market.getSaveInfo();saveGame.Portfolio=Portfolio.getSaveInfo();saveGame.BotShop=BotShop.getSaveInfo();saveGame.Calendar=Calendar.getSaveInfo();store.set('GameSave',saveGame)},60000)})},{"./controller/BotShop.js":7,"./controller/Broker.js":8,"./controller/Calendar.js":9,"./controller/Market.js":10,"./controller/OddJobs.js":11,"./controller/Portfolio.js":12,"./model/Loop.js":15,"./view/Console.js":22}],14:[function(require,module,exports){module.exports=class{constructor(Loop,Portfolio,GameConsole,controller){this.controller=controller;this.GameConsole=GameConsole;this.portfolio=Portfolio.model;this.loop=Loop;this.day=0;this.hour=0;this.minute=0;this.gameLost=!1;this.dailyExpenditures=[{name:"Heating",description:"Make sure you dont freeze to death",reoccuring:1,daysLeft:1,cost:60,hidden:!1},{name:"Electricity",description:"Keep everyones phones charged",reoccuring:1,daysLeft:1,cost:60,hidden:!1},{name:'Buy Food',description:'Eat to live',reoccuring:2,daysLeft:4,cost:50,hidden:!1},{name:"Mortgage",description:"Gotta keep make sure you keep your house",reoccuring:5,daysLeft:10,cost:1000,hidden:!1}]}
...
if((this.hour%8)==0&&this.hour!=0){this.hour=0;this.dayEnd();this.day ++}}
dayEnd(){this.dailyExpenditures.forEach(current=>{current.daysLeft--;if(current.daysLeft==0){if(current.hidden){this.GameConsole.message(`You have been charged $${ current.cost } for ${ current.name }.`)}
...
return tempObj}}},{}],15:[function(require,module,exports){class Loop{constructor(){this.paused=!1;this.views=[];this.repeating=[];this.waiting=[];setInterval(()=>{if(!this.paused){this.runViewUpdate()}},250);setInterval(()=>{if(!this.paused){this.runRepeating()}},100);setInterval(()=>{if(!this.paused){this.runWaiting()}},100)}
...
module.exports=()=>{return new Loop()}},{}],16:[function(require,module,exports){module.exports=class{constructor(){this.stocks=[]}
...
return stockArr}}},{}],17:[function(require,module,exports){module.exports=class{constructor(Portfolio){this.portfolio=Portfolio
this.working=!1;this.jobs=[{name:'Cleaning',payment:10,timeTaken:5,locked:0},{name:'Call Center',payment:25,timeTaken:7,locked:1},{name:'Paperboy',payment:20,timeTaken:4,locked:2},{name:'Milkman',payment:25,timeTaken:2,locked:3}]}
work(job){if(!this.working){this.working=!0;setTimeout(()=>{this.working=!1;this.portfolio.cash+=job.payment},job.timeTaken*1000)}}}},{}],18:[function(require,module,exports){module.exports=class{constructor(cashValue,market,gameConsole){this.gameConsole=gameConsole;this.market=market;this.stocks=[];this.cash=cashValue}
...
getSaveInfo(){let tempObj={}
tempObj.name=this.name;tempObj.price=this.price;tempObj.momentum=this.momentum;tempObj.volatility=this.volatility;tempObj.history=this.history;return tempObj}}},{}],20:[function(require,module,exports){module.exports=class{constructor(market,portfolio,loop,calendar){this.calendar=calendar
...
this.repopulate();if(this.prevDay!=this.calendar.day){this.dayEnd();this.prevDay=this.calendar.day}}}},{"./Help.js":23}],22:[function(require,module,exports){class Console{constructor(){this.consoleDomElement=$('#console')}
message(message){var date=new Date();var newMessage=date.getHours()+":"+date.getMinutes()+" > "+message;this.consoleDomElement.prepend(`<p class="console">${ newMessage }</p>`)}
...
return gameConsole}},{}],23:[function(require,module,exports){arguments[4][4][0].apply(exports,arguments)},{"dup":4}],24:[function(require,module,exports){module.exports=class{constructor(market,loop,calendar){this.calendar=calendar
...
repopulate(){$('#stock-viewer').empty();this.market.iterate((stock)=>{let icon=stock.getDiff()>0?"green fas fa-chevron-up":"red fas fa-chevron-down";$('#stock-viewer').append(`
        <tr>
          <td>${ stock.name }</td>
...
this.portfolio.stocks.forEach(current=>{$('#'+current.stock.name+'PortPrice').text(current.stock.price.toFixed(2));$('#'+current.stock.name+'PortTotal').text((current.quantity*current.stock.price).toFixed(2));$('#'+current.stock.name+'PortQuant').text(current.quantity)})}}},{"./Help.js":23}]},{},[13])
